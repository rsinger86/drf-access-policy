...
| condition     | The name of a method on the policy that returns a boolean or `rest_framework.permissions.BasePermission` subclass import path in format `"import:package.module.Class"`. The method signature is `condition(request, view, action: str)`. If true, the policy will be in effect. Useful for enforcing object-level permissions. If list, all conditions must evaluate to `True`. | `"is_manager_for_account"` <br> `"is_author_of_post"` <br> `["balance_is_positive", "account_is_not_frozen"]` <br> `"import:rest_framework.permissions.IsAdminUser"` |
