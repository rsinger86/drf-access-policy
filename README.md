...
| action     | The action or actions that the statement applies to. Can be formatted as a string or list of strings. The value should match the name of a view set method or the name of the view function. | `"*"` (any action)<br> `"<safe_methods>"` (a read-only HTTP request: HEAD, GET, OPTIONS) | `["list", "delete", "create]` <br> `["*"]` <br> `["<safe_methods>"]`   |
